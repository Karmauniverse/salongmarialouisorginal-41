...
const Testimonials: React.FC = () => {
  ...
  return <section id="testimonials" className="py-16 bg-gradient-to-b from-white to-salon-cream/30 bg-white">
      <div className="section-container">
        ...
        <div ref={el => elementsRef.current[3] = el} className="animated-element relative max-w-4xl mx-auto">
          <div className="relative overflow-hidden bg-white rounded-lg shadow-xl p-8 md:p-12">
            ...
            <div className="relative z-10">
              <div className={cn("transition-all duration-500 ease-in-out", isTransitioning ? "opacity-0 transform scale-95" : "opacity-100 transform scale-100")}>
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="flex">
                      {[...Array(5)].map((_, i) => <Star key={i} size={16} className="text-yellow-400 fill-yellow-400 mr-1" />)}
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="flex justify-center mt-8 space-x-4">
            <button onClick={handlePrev} className="p-3 rounded-full border border-salon-gold text-salon-gold hover:bg-salon-gold hover:text-white transition-colors shadow-md hover:shadow-lg transform hover:scale-105 duration-300" aria-label="Föregående omdöme" disabled={isTransitioning}>
              <ChevronLeft size={20} />
            </button>
            
            <div className="flex space-x-3 items-center">
              {testimonials.map((_, index) => <button key={index} onClick={() => handleDotClick(index)} className={cn("transition-all duration-300 rounded-full", activeTestimonial === index ? "bg-salon-gold w-6 h-2" : "bg-salon-gold/30 w-2 h-2 hover:bg-salon-gold/60")} aria-label={`Gå till omdöme ${index + 1}`} />)}
            </div>
            
            <button onClick={handleNext} className="p-3 rounded-full border border-salon-gold text-salon-gold hover:bg-salon-gold hover:text-white transition-colors shadow-md hover:shadow-lg transform hover:scale-105 duration-300" aria-label="Nästa omdöme" disabled={isTransitioning}>
