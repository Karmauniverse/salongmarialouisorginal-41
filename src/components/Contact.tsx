...
import { MapPin, Phone, Clock, Mail, CreditCard, CalendarDays } from 'lucide-react';
...
const Contact: React.FC = () => {
  ...
  return <section id="contact" className="py-20 bg-gradient-to-b from-white to-salon-cream/50">
      <div className="section-container my-0 py-0 px-0">
          ...
          <div className="order-2 md:order-1">
            <div ref={el => elementsRef.current[3] = el} className="animated-element bg-salon-dark rounded-lg overflow-hidden shadow-xl">
              <div className="p-8 text-white space-y-12">
                <h3 className="text-2xl font-serif mb-8 text-salon-gold">Kontaktinformation</h3>
                ...
                <div className="space-y-6">
                  {contactInfoItems.map((item, index) => <div key={index} className="flex group">
                      ...
                      <div>
                        ...
                        {item.details.map((detail, i) => <p key={i} className="text-white/80">{detail}</p>)}
                      </div>
                    </div>)}
                </div>
          ...
          <div className="order-1 md:order-2">
            <div ref={el => elementsRef.current[4] = el} className="animated-element bg-white rounded-lg shadow-xl p-8 relative overflow-hidden">
              ...
              <div className="space-y-6 relative z-10">
                <a href="https://www.bokadirekt.se" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-full py-4 px-6 bg-salon-gold text-white font-medium rounded hover:bg-opacity-90 transition-all shadow-md hover:shadow-lg transform hover:translate-y-[-2px] group">
                  <CalendarDays className="mr-2 group-hover:animate-pulse" size={20} />
                  <span>Boka Tid via Bokadirekt</span>
                </a>
                
                <div className="text-center my-4 flex items-center justify-center">
                  <div className="flex-grow h-px bg-gray-200"></div>
                  <span className="px-4 text-sm text-gray-400">eller</span>
                  <div className="flex-grow h-px bg-gray-200"></div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input id="name" name="name" value={formState.name} onChange={handleInputChange} className="border-salon-gold/20 focus-visible:ring-salon-gold" placeholder="Ditt namn" required />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="phone">Telefon</Label>
                      <Input id="phone" name="phone" value={formState.phone} onChange={handleInputChange} className="border-salon-gold/20 focus-visible:ring-salon-gold" placeholder="Ditt telefonnummer" required />
                    </div>
                  </div>
                  
                  <div className="space-y-2">
                    <Label htmlFor="email">E-post</Label>
                    <Input id="email" name="email" type="email" value={formState.email} onChange={handleInputChange} className="border-salon-gold/20 focus-visible:ring-salon-gold" placeholder="Din e-postadress" required />
                  </div>
                  
                  <div className="space-y-2">
                    <Label htmlFor="message">Meddelande</Label>
                    <Textarea id="message" name="message" value={formState.message} onChange={handleInputChange} className="border-salon-gold/20 focus-visible:ring-salon-gold min-h-[120px]" placeholder="Vad vill du boka tid för?" required />
                  </div>
                  
                  <button type="submit" className="w-full py-3 px-6 bg-salon-dark text-white font-medium rounded hover:bg-salon-brown transition-all shadow-md">
                    Skicka förfrågan
                  </button>
                </form>
              </div>
