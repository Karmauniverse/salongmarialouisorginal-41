  ...
  return (
    <>
      <nav className={cn(
        ...
        {/* Mobile Navigation */}
        <div className={cn(
          "fixed inset-0 bg-white z-40 flex flex-col pt-24 px-6 transition-all duration-500 ease-in-out md:hidden",
          isMobileMenuOpen 
          ...
          <div className="flex flex-col space-y-6 items-center">
            {navLinks.map(link => (
              <a 
                key={link.name} 
                href={link.href} 
                className="text-salon-dark text-lg font-medium relative after:absolute after:bottom-0 after:left-1/4 after:right-1/4 after:w-1/2 after:mx-auto after:h-0.5 after:bg-salon-gold after:transition-all after:duration-300 hover:after:w-full hover:after:left-0 hover:after:right-0 hover:text-salon-gold pb-1" 
                onClick={toggleMobileMenu}
              >
                {link.name}
              </a>
      ...
      <div className="fixed right-6 bottom-1/3 z-40 flex flex-col gap-3 py-3 px-2 rounded-full bg-white/80 backdrop-blur-sm shadow-md">
        <a 
          href="https://www.instagram.com/salongmarialouiis/" 
          target="_blank" 
          rel="noopener noreferrer" 
          className="text-salon-dark hover:text-salon-gold transition-colors p-2 hover:scale-110 transform duration-300" 
